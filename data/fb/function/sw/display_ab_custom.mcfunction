...
data modify storage fb:tmp config_setup set value {key: "global", target: "@a[tag=!fb.ab.custom]", player: "@a[tag=!fb.ab.custom]", is_global: 1b}
...
execute if data storage fb:tmp check[{name:"@a"}] if data storage fb:tmp global_check[{type:"hard"}] run data modify storage fb:tmp config_setup.player set value "@a"

$execute unless data storage fb:tmp check[{name:"@a"}] run data modify storage fb:tmp config_setup set value {key: "$(player)", target: "$(player)", player: "$(player)", is_global: 0b, global_type: "none"}
