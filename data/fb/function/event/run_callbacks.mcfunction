...
# 1. Store event name in event_check
data modify storage fb:tmp event_check set value {name: ""}
$data modify storage fb:tmp event_check.name set value "$(event)"

# 2. Capture player's held item (Mainhand SelectedItem or Offhand Slot -106b) into fb:tmp event_context.item
...
# 3. Extract live custom_data component compound into fb:tmp item_cd & fb:tmp event_context.item_cd
...
# 4. Special handling: add player to online list on onJoin
...
# 6. Copy registered callbacks array for this event into event_context.list
...
# 7. Diagnostic Warning if no callbacks exist in storage for RightClick ONLY
